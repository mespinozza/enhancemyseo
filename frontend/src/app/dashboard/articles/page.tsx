...
export default function ArticlesPage() {
  ...
  const handleBulkGenerate = async () => {
    // Parse keywords based on mode (bulk textarea or individual inputs)
    const validKeywords = bulkMode 
      ? parseBulkKeywords(bulkKeywordsText)
  ...
  // Helper functions for admin bulk mode
  const toggleBulkMode = () => {
    if (!bulkMode) {
      // Switching to bulk mode - convert existing keywords to text
      const existingKeywords = keywords.filter(k => k.trim().length > 0).join('\n');
      setBulkKeywordsText(existingKeywords);
    } else {
      // Switching to normal mode - convert text to keyword array
      const parsedKeywords = parseBulkKeywords(bulkKeywordsText);
      setKeywords(parsedKeywords.length > 0 ? parsedKeywords : ['']);
    }
  ...
  const handleDownloadAll = () => {
    ...
    generatedArticles.forEach((article, index) => {
      ...
      setTimeout(() => {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `${article.title} - generated by enhancemyseo.txt`;
        document.body.appendChild(a);
        a.click();
        window.URL.revokeObjectURL(url);
        document.body.removeChild(a);
    ...
    toast.success(`Downloading ${generatedArticles.length} article${generatedArticles.length !== 1 ? 's' : ''}...`);
  };
        ...
        <div className="max-w-xl mx-auto">
          ...
          <div className="space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center space-x-1">
                  <label className={`block text-sm font-medium ${!keywords[0]?.trim() && !bulkKeywordsText.trim() && touchedFields.keyword ? 'text-red-600' : 'text-gray-700'}`}>
                    Article Keywords
                    <span className="ml-1 text-red-600">*</span>
                  </label>
                  <div className="relative group">
                    ...
                    <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-64 px-3 py-2 bg-gray-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none">
                      {bulkMode 
                        ? 'Enter keywords for articles (one per line). No limit for admin users.'
                ...
                <div className="flex items-center space-x-2">
                  {/* Admin Bulk Mode Toggle */}
                  {subscription_status === 'admin' && (
                    <button
                      onClick={toggleBulkMode}
                      className="flex items-center space-x-1 px-3 py-1 text-xs bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-full hover:from-purple-700 hover:to-blue-700 transition-all shadow-sm"
                      title={bulkMode ? "Switch to normal mode" : "Switch to bulk mode"}
                    >
                      <Sparkles size={12} />
                      <span>{bulkMode ? 'Normal Mode' : 'Bulk Generate'}</span>
                    </button>
                  )}
                  
                  {/* AI Suggestions (only show in normal mode) */}
                  {!bulkMode && keywords[0]?.trim() && keywords.filter(k => k.trim().length > 0).length >= 1 && keywords.some(k => !k.trim()) && (
                    <button
              ...
              {/* Keyword input fields - Conditional rendering based on mode */}
              {bulkMode ? (
                /* Bulk Mode: Large Textarea */
                <div>
                  <textarea
                    value={bulkKeywordsText}
                    onChange={(e) => setBulkKeywordsText(e.target.value)}
                    onFocus={() => markFieldAsTouched('keyword')}
                    rows={10}
                    className={`block w-full rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 font-mono text-sm ${
                      !bulkKeywordsText.trim() && touchedFields.keyword ? 'border-red-300' : 'border-gray-300'
                    }`}
                    placeholder="Enter keywords (one per line):&#10;&#10;cosmetic dental bonding in Westfield NJ&#10;teeth whitening services&#10;dental implants near me&#10;orthodontic braces for adults&#10;emergency dentist 24/7"
                  />
                  {/* Info about bulk generation */}
                  {(() => {
                    const validCount = parseBulkKeywords(bulkKeywordsText).length;
                    return validCount > 0 && (
                      <p className="mt-2 text-sm text-blue-600 flex items-center space-x-1">
                        <Sparkles size={14} />
                        <span>Bulk generation: {validCount} article{validCount !== 1 ? 's' : ''} will be generated</span>
                      </p>
                    );
                  })()}
                  {/* Error message */}
                  {!bulkKeywordsText.trim() && touchedFields.keyword && (
                ...
                /* Normal Mode: Individual Input Fields */
                <div>
            ...
            <button
              ...
              disabled={isGenerating || (bulkMode ? !bulkKeywordsText.trim() : !keywords[0]?.trim()) || !selectedBrandId || !contentType}
              className={`w-full py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white 
                ${
                  isGenerating || (bulkMode ? !bulkKeywordsText.trim() : !keywords[0]?.trim()) || !selectedBrandId || !contentType
            ...
            >
              {isGenerating 
                ...
                : (bulkMode ? !bulkKeywordsText.trim() : !keywords[0]?.trim())
                ? 'Enter Article Keywords'
                : !contentType
                ? 'Select Content Type'
                : (() => {
                    const validKeywords = bulkMode 
                      ? parseBulkKeywords(bulkKeywordsText).length
                      : keywords.filter(k => k.trim().length > 0).length;
                    return `Generate ${validKeywords} Article${validKeywords !== 1 ? 's' : ''}`;
                  })()}
            </button>
            
            {((!selectedBrandId && touchedFields.brandProfile) || ((bulkMode ? !bulkKeywordsText.trim() : !keywords[0]?.trim()) && touchedFields.keyword) || (!contentType && touchedFields.contentType)) && (
              <p className="text-sm text-red-600 text-center">
                {!selectedBrandId && touchedFields.brandProfile
                  ? 'A brand profile is required'
                  : (bulkMode ? !bulkKeywordsText.trim() : !keywords[0]?.trim()) && touchedFields.keyword
                  ? 'At least one keyword is required'
                  : !contentType && touchedFields.contentType
                  ? 'Content type is required'
                  : ''}
